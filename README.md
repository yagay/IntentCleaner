...
包名 `com.yagay.intentcleaner`，版本码20。基于 libxposed API 102 的意图候选过滤模块。不使用 IFW、不修改系统 XML；Root 仅用于用户点击导出时的只读诊断。

本次目录发现、历史候选隔离、恢复与诊断修复见 [CHANGES-1.4.6.md](CHANGES-1.4.6.md)。旧 CHANGES/AUDIT 为历史记录，不代表本版行为。
...
目录发现使用 MATCH_ALL（文本外同时要求 MATCH_DEFAULT_ONLY）；实际文件查询不使用 MATCH_ALL。类型范围匹配单独标注，不再自动归为受限；权限/启用/导出限制才归受限候选。历史候选默认隐藏，可打开“显示历史候选”；未配置历史最多保留7天，已配置项始终保留管理入口，分类、搜索及已选视图仍有效。两个开关持久化，没有会话特例。“用实际文件检查打开方式”只获取MIME并查询候选，不打开目标、不读取正文、不保存完整URI。以本模块身份查询不保证和所有来源应用的菜单完全相同。

系统确认使用 UID 认证的私有查询返回实际配置摘要；只证明系统查询 Hook，不等于所有选择器效果。已知兼容的19/20版可在严格扫描校验前收到暂停配置，FAILED/RELOADING及未知版本仍不允许；提交不等于生效。诊断导出不等待同步或框架状态查询，保存最近一次观察及时间，再收集原始日志。导出任务由 ViewModel 持有，旋转屏幕不会重新开始；应用被杀后不保证继续导出。
