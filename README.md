...
排序页采用与规则页一致的单一应用列表、吸顶筛选栏和单应用展开交互。勾选加入当前分类的优先顺序，取消勾选移除；“全部”中已优先应用按设定顺序排在前面，未优先应用按名称排列；勾选后进入对应优先位置，取消后回到默认名称顺序。“已优先”按设定顺序排列，“未优先”按名称排列。长按已优先应用可拖动到目标位置，靠近边缘自动滚动，松手保存；原有展开后的上移、下移继续保留。搜索后只调整匹配的优先应用，未显示配置保留原位。排序仍以应用为单位、按分类独立保存。
