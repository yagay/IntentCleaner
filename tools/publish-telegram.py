...
The bot token is read only from TELEGRAM_BOT_TOKEN. A small marker asset is uploaded
back to the GitHub Release after Telegram confirms delivery so reruns do not resend
...
MARKER_NAME = "telegram-published.json"
...
def make_caption(version, body, release_url):
    ...
    caption = "\n".join(parts)
    # Telegram document captions allow up to 1024 characters. Keep headroom for
    # Unicode length differences and future wording changes.
    return caption[:900]
...
def main():
    ...
    if find_asset(release, MARKER_NAME):
        print(f"Telegram already published for {tag}; skipping duplicate send.")
        return
    ...
    with tempfile.TemporaryDirectory(prefix="listcleaner-telegram-") as directory:
        ...
        caption = make_caption(version, release.get("body", ""), release["html_url"])
        ...
        gh("release", "upload", tag, "--repo", SOURCE, str(marker))
        print(f"Published {tag} to Telegram {chat_id}; message_id={message_id}")
